...
async def test_create_like_to_post(web_client, get_token, post_db, collection):
    ...
    response = await web_client.post(route, data=json.dumps(likes_data), headers=headers)
...
@pytest.mark.asyncio
@pytest.mark.parametrize("collection_name", ["likes"])
async def test_create_like_to_post_and_change_to_unlike(web_client, get_token, post_db, collection):
    # setup
